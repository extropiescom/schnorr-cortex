...
const SIGNING_CTX: &'static [u8] = b"good"; //should be settled from polkadot
...
pub unsafe extern "C" fn sr_sign(
	...
	let mut data: [u8; BUFFER_LEN] = [0; BUFFER_LEN];
	...
	data.copy_from_slice(
		&keypair
			.sign_trng(context.bytes(message_bytes), &trng_bytes[..])
			.to_bytes()[..],
	);
...
pub unsafe extern "C" fn sr_getpub(private_key: *const u8) -> *mut u8 {
	...
	data.copy_from_slice(&public_from_secret.to_bytes()[..]);
	sr_data.status = STATUS_OK;
...
pub unsafe extern "C" fn sr_verify(
	...
	if keypair.verify(context.bytes(message_bytes), &signature).is_ok() {
		STATUS_OK
	} else {
		STATUS_NOK
	}
