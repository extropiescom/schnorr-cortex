...
use rand::prelude::*;
use rand::{RngCore, CryptoRng, SeedableRng, Error};
...
impl Keypair {
    ...
    {
       // let seed = Keypair::generate(thread_rng());//ForceModify
       // let cert_secret = self.issue_ecqv_cert(t.clone(), &seed.public);
        ...
        let p = ECQVCertPublic([0u8; 32]);
        let s : SecretKey= SecretKey::from_bytes(&bytes).unwrap();
        (p,s)
    }
}
